...
void AttachSampleToPhylo(struct sample S, struct phylo P, int *SPattach) {
  ...
  //SPattach[sample.taxa#] returns Phylo.taxalist position of that sample taxon
  int i, j;
  int found = 0;
  int error = 0;
  ...
  for (i = 0; i < S.ntaxa; i++) {
    ...
    if (found == 0) {
      error = 1;
      printf("Error (AttachSampleToPhylo): Taxon %s in sample not a terminal taxon in phylogeny.\n",
	     S.taxa[i]);
      if (Debug)
	{
	  printf("\ti=%d, S.ntaxa=%d\n", i, S.ntaxa);
	}
    }
  }
  if (Debug)
  ...
  if (error == 1)
    {
      printf("There were errors in the input. Cannot continue. Exiting.");
      exit(EXIT_FAILURE);
    }
  //if (found == 0) {
  //  printf("Taxon %s in sample not a terminal taxon in phylogeny. i = %d, S.ntaxa = %d, Exiting.\n",
  //   S.taxa[i], i, S.ntaxa);
  //  exit(EXIT_FAILURE);
  //}
...
void ComStruct(phylo InP, sample InS, int SwapMethod, int UseAbund) {
	...
	//output header line
	if (SwapMethod == 3) {
		printf("Phylocom output: randomization method %d, %d swaps, %d runs\n",
				SwapMethod, SWAPS, RUNS);
	} else {
...
void IndependentSwap(sample S, int NumberOfSwaps) {
  ...
  for (swapNum = 0; swapNum < NumberOfSwaps; swapNum++) {
    ...
    while (swapped == FALSE) {
      ...
      randomTaxon1 = ((int) (((double) S.ntaxa * random())
                             / (double)(RAND_MAX)) );
      ...
      randomPlot1 = ((int) (((double) S.nsamples * random())
                            / (double)(RAND_MAX)) );
      randomPlot2 = ((int) (((double) S.nsamples * random())
                            / (double)(RAND_MAX)) );
...
void PhylogenySampleTaxaShuffle(phylo P, sample S, int *attach) {
	...
	for (taxon = 0; taxon<S.ntaxa; taxon++) {
		...
		while (foundTaxon == FALSE) {
			...
			randomSampleTaxon = ((int)(((double)S.ntaxa*random())
					/(double)(RAND_MAX)));
...
void PhylogenyAttachShuffle(phylo P, sample S, int *attach) {
	...
	for (taxon = 0; taxon < P.termtaxa; taxon++) {
		randomPhyloTaxon = ((int)(((double)P.termtaxa*random())
				/(double)(RAND_MAX)));
		tmp = taxaList[taxon];
	...
	free_ivector(taxaList, 0, P.termtaxa-1);
}


// --------- Phylogeny Attach Shuffle ---------------------------
void PhylogenyAttachShuffleFixedPtr(phylo P, sample S, int *attach, int *taxaList) {
  ...
  //NB careful - want terminal taxa(termtaxa?) not internal nodes (ntaxa)
  for (taxon = 0; taxon < P.termtaxa; taxon++) {
    randomPhyloTaxon = ((int)( ((double)(P.termtaxa-1)*random()) / (double)(RAND_MAX) ));
    tmp = taxaList[taxon];
    taxaList[taxon] = taxaList[randomPhyloTaxon];
    taxaList[randomPhyloTaxon] = tmp;
  }
...
void RandomizeSampleTaxaShuffle(sample S) {
	...
	free_ivector(taxaList, 0, S.ntaxa-1);
}

// ---------- Sample ID + Phylo-Attach Shuffle --------------------------------
void RandomizeSampleTaxaShuffleFixedPtr(sample S, int *taxaList) {
  //This method works by assigning each record in the sample file a
  //random sample taxon ID #
  //The sample-phylo attach will need to be subsequently shuffled to
  //give every taxon in the phylo a chance to be included in the samples
  ...
  for (sampleNum = 0; sampleNum<S.nsamples; sampleNum++) {
    for (taxon = 0; taxon < S.ntaxa; taxon++) {
      randomSampleTaxon = ( (int)(( (double)(S.ntaxa-1)*random() ) / (double)(RAND_MAX) ) );
      //printf("RAND_MAX: %d\n", RAND_MAX);
      //printf("rand(): %d\n", rand());
      //printf("random(): %ld\n", random());
      tmp = taxaList[taxon];
...
// Community distance (abundance weighted)
void CommunityDistance(phylo InP, sample InS, int abundWeighted) {
	int j, k, plot1, plot2;
	int *attach;
	double **comDist;

	comDist = dmatrix(0, InS.nsamples-1, 0, InS.nsamples-1);
	...
	if (abundWeighted == 1) {
		...
		for (plot1 = 0; plot1 < InS.nsamples; plot1++) {
			for (plot2 = 0; plot2 < InS.nsamples; plot2++) {
				...
				//calculate weighted mean pairwise distance among taxa
				comDist[plot1][plot2] = plotSum / (double) pairCount;
	...
	//free memory
	free_dmatrix(comDist, 0, InS.nsamples-1, 0, InS.nsamples-1);
...
void RaoDiversity(phylo P, sample S) {
	...
	long numTaxaPresent;
	int *taxaPresent, taxon1, taxon2;
	double *totAbund;
	taxaPresent = ivector(0, S.ntaxa-1);
	totAbund = dvector(0, S.ntaxa-1);
	for (k = 0; k < S.nsamples-1; k++) {
		for (l = k+1; l < S.nsamples; l++) {
			...
			numTaxaPresent = 0;
			for (i = 0; i < S.srec[k]; i++) {
				totAbund[S.id[k][i]] += S.pabund[k][i];
				taxaPresent[numTaxaPresent] = S.id[k][i];
				numTaxaPresent++;
			}
			for (j = 0; j < S.srec[l]; j++) {
				if (totAbund[S.id[l][j]] == 0.0) {
					taxaPresent[numTaxaPresent] = S.id[l][j];
					numTaxaPresent++;
				}
				totAbund[S.id[l][j]] += S.pabund[l][j];
			}
			for (i = 0; i < S.ntaxa; i++)
				totAbund[i] /= 2.0;
			for (i = 0; i < numTaxaPresent; i++) {
				if (Debug)
					printf("Plot %d %d %f %f %f\n", k, l, S.pabund[k][i],
							S.pabund[l][i], totAbund[i]);
				for (j = 0; j < numTaxaPresent; j++) {
					taxon1 = taxaPresent[i];
					taxon2 = taxaPresent[j];
					if (taxon1 != taxon2) {
						H[k][l] += totAbund[taxon1] * totAbund[taxon2];
						Hp[k][l] += (TaxaDist(P, taxon1, taxon2, attach) / 2.0)
								* totAbund[taxon1] * totAbund[taxon2];
					}
				}
			}
			H[k][l] = ((2.0 * H[k][l]) - D[k] - D[l]) * 2.0;
			Hp[k][l] = ((2.0 * Hp[k][l]) - Dp[k] - Dp[l]) * 2.0;
		}
	}
	...
	if (S.nsamples > 0) {
		for (i = 1; i<S.nsamples; i++) {
			if (S.irec[i] != S.irec[i-1]) {
				printf("#Warning - diversity components and among-community diversity are only interpretable when all communities contain equal numbers of individuals. See manual for details.\n");
				break;
			}
		}
	}
	...
	printf("#----- AMONG-COMMUNITY DIVERSITY\n");
	printf(".");
	for (k = 0; k < S.nsamples; k++)
		printf("\t%s", S.pname[k]);
	printf("\n");
	...
	printf("#----- AMONG-COMMUNITY PHYLOGENETIC DIVERSITY\n");
	printf(".");
	for (k = 0; k < S.nsamples; k++)
		printf("\t%s", S.pname[k]);
	printf("\n");
	...
	free_ivector(taxaPresent, 0, S.ntaxa-1);
	free_dvector(totAbund, 0, S.ntaxa-1);
