...
int main(int argc, char *argv[])
{
  ...
  OUTMODE = OUT_DIFFCOEFFS_E; //Setting default to "all ELD coeffifients"
  OutputLttrRawFlag = 0;
  OutputVariancesFlag = 0;
  strcpy(PhyloFile, "phylo");
  strcpy(SampleFile, "sample");
  strcpy(TraitFile, "traits");
  strcpy(LttrIntvsFile, "lttrsplits");
  ...
  for (argx = 0; argx <  argc; argx++)
      ...
      if (strcmp(argv[argx], "-s") == 0)
	{
      ...
      // lttr points file
      if (strcmp(argv[argx], "-z") == 0)
	{
	  sscanf(argv[argx+1], "%s", LttrIntvsFile);
	}
      ...
      if (strcmp(argv[argx], "-o") == 0)
	{
      ...
      if (strcmp(argv[argx], "-ol") == 0)
	{
	  OutputLttrRawFlag = 1;
	}
      if (strcmp(argv[argx], "-i") == 0)
        {
          OutputVariancesFlag = 1;
        }
      if (strcmp(argv[argx], "-a") == 0)
	{
	  sscanf(argv[argx+1], "%d", &OUTMODE);
	  if (OUTMODE < 0) OUTMODE = 0;
	  if (OUTMODE >= OUT_LASTMODE) OUTMODE = 0;
	}
  ...
  else if (strcmp(Method, "comdistnew") == 0)
    {
      CommunityDistance(ReadPhylogeny(PhyloFile), ReadSample(SampleFile), UseAbund);
      return 1;
    }
  ...
  else if (strcmp(Method, "ltteld") == 0)
    {
      LttRv2(ReadPhylogeny(PhyloFile), ReadSample(SampleFile), ReadLttrIntv(LttrIntvsFile));
      return 1;
    }
