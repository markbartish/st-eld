...
phylo New2fy(char filename[50]) {
	...
	while (i < lng) {
		...
		if ( ( ( (line[i] >= 65) && (line[i] <= 90)) ||( (line[i] >= 97)
				&& (line[i] <= 122))||(line[i] == 45)|| (line[i] == 95))
				&&(strncmp(&line[i-1], ")", 1) == 0)) {
			// clear iname
			for (q = 0; q < MAXTAXONLENGTH; q++)
				iname[q] = 0;

			while ( (strncmp(&line[i], ":", 1) != 0) &&(strncmp(&line[i], ",",
					1) != 0)&&(strncmp(&line[i], ")", 1) != 0)&&(strncmp(
					&line[i], "[", 1) != 0)&&(strncmp(&line[i], ";", 1) != 0)
					&&(strncmp(&line[i], "]", 1) != 0)) {
		...
		//TODO - fix this. anytime you find a non ,:;()[] character, check to see if
		//previous VALID character is , or ). If it is, or if character is ', begin a new
		//taxon name. If it's not, ignore. This should handle whitespace, newlines and
		//other garbage (but non-fatal) characters that currently crash the input.
		if (done == 0) {
			for (q = 0; q < 100; q++)
				taxa[q] = 0;
